"""Validate wrist-camera temporal evidence on a real DROID episode."""
import argparse
import numpy as np
...
def main():
    ...
    ev=analyze_wrist_sequence(
        frames,reference_index=a.reference_frame,
        start=max(0,a.grasp_frame-10),end=min(len(frames)-1,a.release_frame+10),
        threshold=a.threshold,
    )
    strongest=sorted(ev,key=lambda x:x.change_score,reverse=True)[:a.top]

    print("=== REAL WRIST TEMPORAL VISION ===")
    print("Task:",instruction)
    print("Reference frame:",a.reference_frame)
    print("Manipulation window:",a.grasp_frame,"->",a.release_frame)
    print()
    print("Strongest visual-change frames:")
    for x in strongest:
        c="None" if x.centroid is None else f"({x.centroid[0]:.1f}, {x.centroid[1]:.1f})"
        print(f"frame={x.frame_index:3d} change={x.change_score:7.2f} changed={x.changed_fraction:.3f} centroid={c}")
    print()
    print("WRIST TEMPORAL ANALYSIS: PASS")
